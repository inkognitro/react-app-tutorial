...
### 4.3 Create the JSON file translator
...
You could have different translations for different apps. This is the reason we define our translation files
...
## 4.4 Enable `ReactNode` placeholders
Let's imagine that you have a translation `"Hi {{username}}!"` at the `IndexPage`.
You have implemented the translations like we have done it before,
...
You start to implement it, and then you realize: the current solution only supports string placeholders :fearful:

So let's write a React component to make your life easier by supporting `ReactNode` placeholders:
...
## 4.5 Using `ReactNode` placeholders
So now that we support ReactNode placeholders, we should verify that everything works as expected. 
...
```
Make sure, that you have added the section above and didn't replace the current content with it.
...
Well done! Check your browser at [localhost:3000](http://localhost:3000). Looks like a bold username.

:floppy_disk: [branch 04-i18n-1](https://github.com/inkognitro/react-app-tutorial-code/compare/03-routing-2...04-i18n-1)

## 4.6 Clean up the body's css with MUI instead
...
## 4.7 Time to add another language
