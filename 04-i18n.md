...
Then provide the services, so that the return statement looks like so:
```typescript jsx
// src/ServiceProvider.tsx
...
```
Then we should provide the `BrowserI18nManager` with the `<I18nManagerProvider>`.
If you don't know exactly how to do this, take the `<TranslatorProvider>` as a reference.

If you still face some problems, just have a look at the
[changes](https://github.com/inkognitro/react-app-tutorial-code/compare/04-i18n-1...04-i18n-2)
in the `ServiceProvider.tsx`.

> :bulb: It is up to you to define how the logic should look like when to choose which translation file.
> It could probably make more sense to choose an `en.json` file, no matter which country code is present in the
> `nextI18nState.languageCode`.

Next let's add following file changes to our `<TestServiceProvider>` component file:
```typescript jsx
// src/TestServiceProvider.tsx
...
Now that the I18nManager is available for all the child components, we can initialize the current language
...
```
This one detects the current user's language, either the chosen one or the browser's default language,
and sets the specific dictionary in the `DictionaryTranslator` (see logic in the `ServiceProvider`).
