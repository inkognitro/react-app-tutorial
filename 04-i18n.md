...
Your Navigation now should take the values from `enUS.json`.
...
Before we check that it works as expected, we should add this greeting translation to our existing translations:
```json
// src/components/translations/enUS.json
...
```
Make sure you've added the section above without overwriting the existing content of the file.

Let's use the `<T>` component in our `IndexPage`. The file should look like so:
```typescript
...
> And also keep in mind: Never clone a React component because this leads to performance issues!
